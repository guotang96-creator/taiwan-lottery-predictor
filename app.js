document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadData() {
    if (!gameEl || !historyCountEl) return;
    try {
      setStatus("讀取中...");
      ...
      setStatus(`已讀取 ${historyData.length} 期`);
    } catch (e) {
      console.error(e);
      historyData = [];
      setStatus("讀取失敗");
  ...
  function analyzeData() {
    ...
    renderAiScoreBoard(aiRank, gameEl ? gameEl.value : "");
  ...
  function goBattle() {
    if (!resultEl || !resultEl.innerHTML.trim()) {
      setStatus("請先分析後再進入實戰");
      return;
    }

    setStatus("實戰模式已啟動，請依推薦號碼作為參考使用");
    window.scrollTo({ top: 0, behavior: "smooth" });
  }

  function getConfig() {
    const game = gameEl ? gameEl.value : "539";

    if (game === "bingo") {
      return { game, max: 80, pickCount: Number(bingoPickCountEl?.value || 6) };
    }
    if (game === "lotto") {
      return { game, max: 49, pickCount: 6 };
    }
    if (game === "power") {
      return { game, max: 38, pickCount: 6 };
    }
    return { game, max: 39, pickCount: 5 };
  }
  ...
  function getBirthdayNumbers(max) {
    ...
    return [...new Set(
      nums.map(n => normalizeNumber(n, max)).filter(n => n >= 1 && n <= max)
    )].sort((a, b) => a - b);
  }
  ...
  function buildBestCombo(aiRank, pickCount) {
    return aiRank.slice(0, pickCount).map(v => v.n).sort((a, b) => a - b);
  }
